...
from yowoformer.model.fusion.cfam import CFAMFusion
from yowoformer.model.head.dfl import DFLHead
from yowoformer.model.backbone3d.build import build_backbone3D
from yowoformer.model.backbone2d.build import build_backbone2D
from yowoformer.model.fusion.build import build_fusion_block
...
class YOWOFormer(torch.nn.Module):
    def __init__(self, num_classes, backbone2D, backbone3D, interchannels, mode, img_size, pretrain_path=None,
                 freeze_bb2D=False, freeze_bb3D=False, fusion_block='CFAM', clip_length=16):
        super().__init__()
        ...
        # Use clip_length from config (supports both 8 and 16 frames)
        dummy_img3D = torch.zeros(1, 3, clip_length, img_size, img_size)
...
def build_yowoformer(config):
    ...
    clip_length   = config.get('clip_length', 16)  # Support 8 or 16 frames
    ...
    return YOWOFormer(num_classes, backbone2D, backbone3D, interchannels, mode, img_size, pretrain_path,
                     freeze_bb2D, freeze_bb3D, fusion_block, clip_length)